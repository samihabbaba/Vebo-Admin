...
@Component({
  selector: 'app-online-customer',
  templateUrl: './online-customer.component.html',
  styleUrls: ['./online-customer.component.scss'],
})
export class OnlineCustomerComponent implements OnInit {
  ...
  LoadUsers() {
    ...
    this.dataService.getUsers('Online', this.searchInput).subscribe(
      (response) => {
  ...
  initializeAddForm() {
    this.addForm = this.fb.group({
      ...
      localComissionSetting: new FormControl(true, [Validators.required]),
      minimumOdd: new FormControl(1.5, [Validators.required]),

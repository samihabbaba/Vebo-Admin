...
@Component({
  ...
  templateUrl: './bet-office.component.html',
  styleUrls: ['./bet-office.component.scss'],
})
export class BetOfficeComponent implements OnInit {
  CurrencyList = [
    { name: 'TL', symbol: '₺' },
    { name: 'USD', symbol: '$' },
    { name: 'STG', symbol: '£' },
  ...
  ngOnInit() {
    if (this.dataService.currentUser.role === 'Shop') {
  ...
  LoadUsers() {
    ...
    this.dataService.getUsers('Office', this.searchInput).subscribe(
      (response) => {
